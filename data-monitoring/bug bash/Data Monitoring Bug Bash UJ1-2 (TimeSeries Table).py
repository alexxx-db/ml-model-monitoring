...
# MAGIC %md 
# MAGIC 
# MAGIC # Data Monitoring Bug Bash UJ1-2 (TimeSeries Table)
# MAGIC 
...
table_name = f"dm_bugbash.default.{sanitized_user}_time_series" 

# COMMAND ----------

# MAGIC %md ##0. Prepare monitored table
# MAGIC You will need to be a **UC table OWNER** in order to create monitor. If you don't have a time sereis table that you can test for monitoring, here are some time series table dataset examples: 
# MAGIC 
# MAGIC <br/>
# MAGIC 
# MAGIC - **TV sales dataset**:  `/dbfs:/user/hive/warehouse/automl_benchmark.db/tv_sales`. 
# MAGIC 
# MAGIC ```
# MAGIC import pandas as pd
# MAGIC df = spark.read.table(
# MAGIC   "hive_metastore.automl_benchmark.tv_sales")
# MAGIC df.write.mode("overwrite").format("delta").option("delta.enableChangeDataFeed", "true").saveAsTable(table_name)
# MAGIC ```
# MAGIC <br/>
# MAGIC 
# MAGIC - **NYC taxi dataset**: 
# MAGIC The nyctaxi database contains the table trips, which has details about taxi rides in New York City stored using Delta Lake ([src](https://www.nyc.gov/site/tlc/about/tlc-trip-record-data.page))
# MAGIC ```
# MAGIC 
# MAGIC spark.sql(f"CREATE TABLE {table_name} DEEP CLONE samples.nyctaxi.trips TBLPROPERTIES (delta.enableChangeDataFeed = true)")
# MAGIC ```
...
time_series_analysis = analysis_type.TimeSeries(
  # UJ: Fill the parameters to create time-series analysis type.
)
dm_info = dm.create_monitor(
  # UJ: Fill the parameters to create the monitor with time-series analysis type.
)
